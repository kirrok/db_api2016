CREATE DATABASE  IF NOT EXISTS `TPForum` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `TPForum`;
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`Alexandra`@`localhost`*/ /*!50003 TRIGGER `TPForum`.`post_AFTER_INSERT` AFTER INSERT ON `post` FOR EACH ROW
BEGIN
	IF (SELECT COUNT(*) FROM user_forum WHERE user = NEW.user AND forum = NEW.forum) = 0 THEN
		INSERT INTO user_forum (user,forum) VALUES(NEW.user, NEW.forum);
    END IF;
END */;;
DELIMITER ;
...
-- Dump completed on 2016-05-08 12:18:01
